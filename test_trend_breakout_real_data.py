"""
REAL DATA HISTORICAL BACKTEST - TREND BREAKOUT & MOMENTUM ENGINE
"""
...
print("=" * 80)
print("REAL DATA BACKTEST - TREND BREAKOUT ENGINE (MAY-JULY 2026 REAL DATA)")
print("=" * 80)
...
for asset in WATCHLIST:
    ...
    if not df.empty:
        ...
        df["H3D"]    = df["High"].shift(1).rolling(48).max()
        df["L3D"]    = df["Low"].shift(1).rolling(48).min()
...
for ts in all_timestamps:
    ...
    for sym, (asset, df) in data.items():
        ...
        # Trend Breakout Strategy: Trade WITH momentum when breaking 2-Day High/Low
        buy_sig  = (close > ema200) and (close > h3d) and (rsi > 55) and (adx > 22)
        sell_sig = (close < ema200) and (close < l3d) and (rsi < 45) and (adx > 22)

        for action, raw in [("BUY", buy_sig), ("SELL", sell_sig)]:
            if not raw or hourly_trades[action]: continue
...
# Performance Summary
print("\n" + "=" * 80)
print("REAL DATA BACKTEST RESULTS - TREND BREAKOUT ENGINE")
print("=" * 80)
