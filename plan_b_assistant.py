"""
===============================================================================
TRADING BOT - STAGE 3: PLAN B ASYMMETRIC AI SIGNAL ASSISTANT
===============================================================================
Engineered for a $200 account with strict $4.00 Risk (2.0%) to $10.00 Reward (5.0%)
and Layered Risk-Free Exit Protection (TP1 + Breakeven + TP2).

Plan B Confluence Rules:
  1. 200-EMA Trend Alignment : BUY only above 200-EMA, SELL only below 200-EMA
  2. Key Channel Pullback     : Touch/Sweep of 3-Day High/Low channels (72 1H candles)
  3. RSI Reversal Extreme    : RSI-14 < 30 for BUY, RSI-14 > 70 for SELL
  4. ADX Regime Filter       : ADX-14 < 30 (Channel Regime)
  5. Session Window          : London (08:00-12:00) & NY (13:30-17:00 London Time)

Layered Exit Protection Rule:
  - TP1 (+1:1 R/R = +$4.00): Scale-out 50% position + Move SL to Breakeven ($0.00 risk!)
  - TP2 (+1:2.5 R/R = +$10.00): Scale-out remaining 50% position for full profit.
===============================================================================
"""
...
print("=" * 90)
print(f"[START] INITIALIZING PLAN B ASYMMETRIC AI SIGNAL ASSISTANT FOR $200 ACCOUNT")
...
if not asset_data_dict:
    raise ValueError("[ERROR] Could not fetch data for watchlist assets.")

print("[OK] Downloaded and processed indicator data for watchlist assets!")
...
def print_sample_signal_card(asset_info, action, entry, sl, tp1, tp2, time_str, rsi_val, adx_val, ema_val):
    ...
    print("\n" + "═" * 75)
    print("🏆 PLAN B HIGH-PROBABILITY AI SIGNAL ALERT 🏆")
    print("═" * 75)
    ...
    print(f"• Stop-Loss (SL)  : {sl:.4f}  --> Risk: -${risk_usd:.2f} (2.0% STRICT RISK)")
    print(f"• Take-Profit 1   : {tp1:.4f}  --> TP1 (50% scale-out + Move SL to Breakeven)")
    print(f"• Take-Profit 2   : {tp2:.4f}  --> TP2 (50% scale-out for +${reward_usd:.2f} total win)")
    print("─" * 75)
    print(f"• Plan B Confluence Rationale:")
    ...
    print(f"  4. Session      : Active Session Window (London / New York Open)")
    print("═" * 75)
...
# =============================================================================
# STEP 4: 2-YEAR PORTFOLIO BACKTEST SIMULATOR (PLAN B RULES)
# =============================================================================
print("[SIMULATION] Running 2-Year Plan B Portfolio Simulation across Watchlist...")
...
for i in range(200, len(sample_df)):
    ...
    for trade in active_trades:
        ...
        if t_type == "BUY":
            # Check TP1: Scale out 50% + SL to Breakeven
            ...
            if bar['High'] >= tp2:
                pnl_part = risk_usd * 0.5 * 2.5  # 1:2.5 R/R on remaining 50%
                current_balance += pnl_part
    ...
    # Active Session Windows: London Open (8-12) & NY Open (13-17)
    ...
    for asset_info in WATCHLIST:
        ...
        # PLAN B CONFLUENCE SIGNALS
        # BUY: Price > 200-EMA (Uptrend) AND Low <= Low_3D AND RSI < 30
        buy_signal = (bar['Close'] > ema_val) and (bar['Low'] <= l3) and (rsi_val < RSI_BUY_MAX)

        # SELL: Price < 200-EMA (Downtrend) AND High >= High_3D AND RSI > 70
        ...
        if buy_signal:
            ...
            tp2_price = bar['Close'] + (2.5 * sl_dist)
...
monthly_avg_return = (total_trades / max(1.0, total_days / 30.4))
...
print("\n" + "=" * 90)
print("📊 PLAN B ASYMMETRIC AI SIGNAL ASSISTANT PERFORMANCE DASHBOARD ($200 Account)")
print("=" * 90)
print(f"• Backtest Duration       : 2 Years ({total_days} Days / {weeks:.1f} Weeks)")
print(f"• Starting Balance        : ${INITIAL_BALANCE:,.2f}")
print(f"• Final Balance           : ${current_balance:,.2f}")
print(f"• Total Net Profit        : ${net_profit:,.2f} ({(net_profit/INITIAL_BALANCE)*100:+.2f}%)")
print(f"• Monthly Average Profit  : ${net_profit / (total_days / 30.4):,.2f} / month (+{((net_profit/INITIAL_BALANCE)/ (total_days / 30.4))*100:.1f}% / month)")
...
print(f"• Maximum Drawdown        : {max_dd_pct:.2f}% (Ultra-Safe Protection!)")
...
# =============================================================================
print("[PLOTS] Generating Plan B Portfolio Equity Curve Chart...")

fig, ax = plt.subplots(figsize=(13, 7))

ax.plot(equity_times, equity_curve, color='#2E7D32', linewidth=2.0, label='Plan B Account Equity ($)')
...
ax.set_title("2-Year Account Equity Curve - Plan B Asymmetric AI Signal Assistant ($200 Account)", fontsize=14, fontweight='bold')
...
chart_file = "plan_b_equity.png"
plt.savefig(chart_file, dpi=150)
print(f"[SAVE] Plan B Equity Curve chart saved to: {os.path.abspath(chart_file)}")

print("\n" + "=" * 80)
print("🏆 FINAL VERDICT & STEP 1 DEMO READINESS")
print("=" * 80)
if net_profit > 0 and profit_factor > 1.2:
    print(f"🎉 SUCCESS! Plan B is PROFITABLE (${net_profit:,.2f} Net Profit, {profit_factor:.2f} Profit Factor, {max_dd_pct:.2f}% Max DD).")
    print(f"👉 System is READY for Step 1 Live Demo Testing!")
else:
    print(f"⚠️ NEEDS TUNING: Net Profit is ${net_profit:,.2f}.")
print("=" * 80 + "\n")

print("✨ Plan B AI Signal Assistant Engine execution completed successfully!")
