"""
REAL DATA HISTORICAL BACKTEST - WITH FULL 11-FILTER DIAMOND & ADAPTIVE SHIELD
"""
...
print("=" * 80)
print("REAL DATA BACKTEST - WITH FULL 11-FILTER DIAMOND & ADAPTIVE SHIELD")
print("=" * 80)
...
trades = []
consec_losses = 0
active_positions = {}  # timestamp -> list of active currencies
...
for ts in all_timestamps:
    ...
    # Correlation Lock: Only allow 1 trade per direction per hour across all pairs!
    hourly_trades = {"BUY": False, "SELL": False}

    for sym, (asset, df) in data.items():
        if ts not in df.index:
            continue
        idx = df.index.get_loc(ts)
        if idx < 100 or idx >= len(df) - 36:
            continue
        ...
        # Strict Multi-Filter Rules (RSI < 30 / > 70 + ADX > 20 + Clean Channel touch)
        buy_sig  = (close > ema200) and (float(bar["Low"]) <= l3d) and (rsi < 32) and (adx > 18)
        sell_sig = (close < ema200) and (float(bar["High"]) >= h3d) and (rsi > 68) and (adx > 18)

        for action, raw in [("BUY", buy_sig), ("SELL", sell_sig)]:
            if not raw: continue

            # D3 Correlation Lock: Prevent taking multiple trades in same hour
            if hourly_trades[action]:
                continue
...
# Performance Summary
print("\n" + "=" * 80)
print("REAL DATA BACKTEST RESULTS (WITH DIAMOND CORRELATION LOCK & STRICT SHIELD)")
print("=" * 80)
...
print(f"  Total Trades        : {total_trades} (Filtered)")
...
print("\nCOMPLETE TRADE LOG:")
print(f"{'#':<4}{'Time':<18}{'Pair':<10}{'Action':<8}{'Result':<12}{'P&L':>10}{'Balance':>12}")
for i, t in enumerate(trades, 1):
    t_str = str(t['time'])[:16]
    print(f"  {i:<4}{t_str:<18}{t['name']:<10}{t['action']:<8}{t['result']:<12}${t['pnl']:>+7.2f}  ${t['balance']:>10.2f}")
